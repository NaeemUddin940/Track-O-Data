    ...
    <div className="bg-white rounded-lg p-6">
      ...
      <div className="grid grid-cols-5 gap-6">
        {brands.map((brand, index) => (
          <div
            key={index}
            className="flex items-center justify-center p-4 hover:bg-gray-50 rounded transition-colors cursor-pointer"
          >
            <img
              src={brand.logo}
              alt={brand.name}
              className={`${brand.width} h-auto object-contain filter grayscale hover:grayscale-0 transition-all`}
            />
          </div>
        ))}
      </div>
